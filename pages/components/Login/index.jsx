  ...
  return (
    <div className="">
      <div className="flex justify-between m-[1rem] mb-[4rem] sm:mb-[2rem]">
        <h1 className="text-[3rem] font-extrabold text-amber-500 sm:text-[2rem] fold:text-[1.5rem] text-center">
          Total {user.length} Users
        </h1>
        <button
          className="logout_btn sm:text-[1rem] fold:text-[0.5rem]"
          onClick={logOut}
        >
          Log Out
        </button>
      ...
      <div className=" flex justify-end m-[1rem]">
        <Link
          href="/components/Login/CreateUser"
          className="create_btn sm:text-[1rem] fold:text-[0.5rem]"
        >
          Create User
        </Link>
      </div>
        ...
        {user.map((user, index) => {
          return (
            ...
            >
              <div className=" ">
                <img
                  className="rounded-full w-[100px] h-[100px] sm:w-[50px] sm:h-[50px] text-center "
                  src={user.avatar}
                  alt={user.avatar}
                />
              </div>
              ...
              <div className="flex justify-center pl-[1rem] cursor-pointer">
                <Link href={`/components/Login/${user.id}`}>
                  <BiEdit className=" text-green-500 mr-[10px] text-[2rem] sm:text-[1rem]" />
                </Link>
                <BiTrash
                  onClick={() => deleteUser(user.id)}
                  className=" text-red-500 text-[2rem] sm:text-[1rem]"
                />
              </div>
            </div>
          );
